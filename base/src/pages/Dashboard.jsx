...
const RISK_COLORS = {
  Low: "#0E9F9A", // Normal wellness teal
  Moderate: "#D9A600", // Attention needed amber
  High: "#E8786D", // Urgent welfare
  Critical: "#C4453D" // Urgent welfare red
};
...
export default function Dashboard() {
  ...
  const handleDownloadAllReport = () => {
    ...
    link.setAttribute("download", `MindPluze_Complete_Dashboard_Report_${new Date().toISOString().slice(0, 10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white">
        <Loader2 className="w-10 h-10 text-[#0E9F9A] animate-spin mb-3" />
        <p className="text-sm font-semibold text-[#081C35]">Loading MindPluze Dashboard...</p>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-[#E8F3FC]/30">
      <SiteNav />
      <Chatbot />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="w-2.5 h-2.5 rounded-full bg-[#0E9F9A] animate-pulse" />
              <span className="text-xs font-bold uppercase tracking-wider text-[#0E9F9A]">
                NHAA 14566 Clinical Module
              </span>
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-[#081C35] tracking-tight">
              My Dashboard
            </h1>
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            <button
              ...
              className="inline-flex items-center gap-2 bg-white border border-slate-200 text-[#081C35] hover:bg-[#E8F3FC] font-semibold px-4 py-2.5 rounded-xl shadow-sm text-sm transition cursor-pointer"
            >
              <BarChart3 className="w-4 h-4 text-[#0E9F9A]" /> View Distribution
            </button>
            <button
              onClick={handleDownloadAllReport}
              className="inline-flex items-center gap-2 bg-[#0E9F9A] hover:bg-[#0C8783] text-white font-semibold px-4 py-2.5 rounded-xl shadow-md shadow-teal-900/10 text-sm transition cursor-pointer"
            >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 mb-8">
          <StatCard icon={Users} label="Total Assessments" value={total} color="#081C35" />
          <StatCard icon={AlertTriangle} label="Critical Risk Cases" value={critical} color="#C4453D" />
          <StatCard icon={Activity} label="Average SVI Score" value={`${avgSvi}/100`} color="#D9A600" />
          <StatCard icon={TrendingUp} label="Languages Screened" value={Object.keys(langCounts).length} color="#0E9F9A" />
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 sm:p-7">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="font-extrabold text-[#081C35] text-lg">Risk Distribution</h3>
                <p className="text-xs text-slate-400">Stress Vulnerability Index breakdown</p>
              </div>
              <button
                onClick={() => setShowDistributionModal(true)}
                className="text-xs font-bold text-[#0E9F9A] hover:underline flex items-center gap-1"
              >
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 sm:p-7">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="font-extrabold text-[#081C35] text-lg">Assessments by Language</h3>
            ...
            {langData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={250}>
                <BarChart data={langData}>
                  ...
                  <Tooltip />
                  <Bar dataKey="value" fill="#0E9F9A" radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-5 mb-5">
          <div className="flex flex-col sm:flex-row gap-3.5 sm:items-center justify-between">
            <div className="flex items-center gap-2.5 bg-[#E8F3FC]/50 border border-slate-200/70 rounded-2xl px-4 py-2.5 flex-1 max-w-md">
              ...
              {["All", "Low", "Moderate", "High", "Critical"].map((f) => (
                <button
                  ...
                  className={`px-3.5 py-2 rounded-xl text-xs font-bold transition cursor-pointer ${
                    filter === f
                      ? "bg-[#081C35] text-[#0E9F9A] shadow-sm"
                      : "bg-[#E8F3FC] text-slate-700 hover:bg-[#D9F6EF]"
                  }`}
                >
                  {f}
                </button>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden mb-6">
          <div className="p-4 sm:px-6 border-b border-slate-100 flex items-center justify-between flex-wrap gap-2">
            <h2 className="font-extrabold text-[#081C35] text-base flex items-center gap-2">
              <FileText className="w-4 h-4 text-[#0E9F9A]" />
          ...
          {filtered.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="bg-[#E8F3FC]/50 text-slate-500 text-xs uppercase tracking-wider font-semibold border-b border-slate-100">
                  <tr>
                    <th className="px-4 py-3.5 w-10">
                      <input
                        ...
                        className="rounded border-slate-300 text-[#0E9F9A] focus:ring-[#0E9F9A] cursor-pointer"
                      />
                    </th>
                    <th className="px-4 py-3.5 font-bold text-[#081C35]">Reference ID</th>
                    <th className="px-4 py-3.5 font-bold text-[#081C35]">Complainant</th>
                    <th className="px-4 py-3.5 font-bold text-[#081C35]">Concern</th>
                    <th className="px-4 py-3.5 font-bold text-[#081C35]">Language</th>
                    <th className="px-4 py-3.5 font-bold text-[#081C35]">SVI Score</th>
                    <th className="px-4 py-3.5 font-bold text-[#081C35]">Risk Level</th>
                    <th className="px-4 py-3.5 font-bold text-[#081C35]">Date</th>
                    <th className="px-4 py-3.5 font-bold text-[#081C35] text-right">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {filtered.map((a) => (
                    <tr
                      key={a.id}
                      className="hover:bg-[#E8F3FC]/40 transition group cursor-pointer"
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 font-mono text-xs font-bold text-[#0E9F9A]">
                        {a.reference_id}
                      </td>
                      ...
                      <td className="px-4 py-3.5 font-extrabold text-[#081C35]">
                        {a.svi_score}
                      </td>
                      <td className="px-4 py-3.5">
                        <span
                          className={`px-2.5 py-1 rounded-full text-xs font-bold inline-block ${
                            a.risk_category === "Low"
                              ? "bg-[#D9F6EF] text-[#0E9F9A]"
                              : a.risk_category === "Moderate"
                              ? "bg-[#FFF0C2] text-[#9A7000]"
                              : "bg-[#FCE1E0] text-[#C4453D]"
                          }`}
                        >
                          {a.risk_category}
                        </span>
                      ...
                      <td className="px-4 py-3.5 text-right whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                        <div className="inline-flex items-center gap-1.5">
                          ...
                          <button
                            onClick={() => setSelectedAssessment(a)}
                            className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-[#E8F3FC] hover:bg-[#0E9F9A] text-[#081C35] hover:text-white text-xs font-bold transition cursor-pointer"
                            title="View Details"
                          >
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-center sm:text-left">
            <h3 className="font-extrabold text-[#081C35] text-base">Dashboard Actions</h3>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            ...
            <button
              onClick={() => setShowDistributionModal(true)}
              className="inline-flex items-center gap-2 bg-[#E8F3FC] hover:bg-[#D9F6EF] text-[#081C35] font-bold px-5 py-3 rounded-2xl text-sm transition shadow-sm cursor-pointer"
            >
              <Eye className="w-4 h-4 text-[#0E9F9A]" /> View Distribution Details
            </button>

            {/* Download Complete Report Button */}
            <button
              onClick={handleDownloadAllReport}
              className="inline-flex items-center gap-2 bg-[#0E9F9A] hover:bg-[#0C8783] text-white font-bold px-5 py-3 rounded-2xl text-sm transition shadow-lg shadow-teal-900/10 cursor-pointer"
            >
              <Download className="w-4 h-4" /> Download Complete Report
            </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="relative w-full max-w-2xl bg-white rounded-3xl p-6 sm:p-8 shadow-2xl border border-slate-100 max-h-[85vh] overflow-y-auto">
            ...
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 rounded-2xl bg-[#E8F3FC] text-[#0E9F9A] flex items-center justify-center">
                <BarChart3 className="w-6 h-6" />
              </div>
              <div>
                <h3 className="text-2xl font-black text-[#081C35]">Clinical Distribution Details</h3>
            ...
            <div className="mb-6">
              <h4 className="text-sm font-bold text-[#081C35] uppercase tracking-wide mb-3">
                ...
                {Object.entries(counts).map(([cat, count]) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-2xl font-black text-[#081C35] mt-1">{count}</p>
                  ...
                  {topIndicators.map(({ name, count }) => {
                    ...
                    return (
                      <div key={name} className="space-y-1">
                        <div className="flex justify-between text-xs font-bold text-[#081C35]">
                        ...
                        <div className="w-full h-2.5 rounded-full bg-slate-100 overflow-hidden">
                          <div
                            className="h-full bg-gradient-to-r from-[#0E9F9A] to-[#C4453D] rounded-full transition-all duration-500"
            ...
            <div className="pt-4 border-t border-slate-100 flex justify-end gap-2">
              <button
                onClick={handleDownloadAllReport}
                className="inline-flex items-center gap-2 bg-[#0E9F9A] text-white text-xs font-bold px-4 py-2.5 rounded-xl hover:bg-[#0C8783] transition shadow"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="relative w-full max-w-2xl bg-white rounded-3xl p-6 sm:p-8 shadow-2xl border border-slate-100 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between pr-8 mb-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-[#0E9F9A]">
                  {selectedAssessment.reference_id}
                </p>
                <h3 className="text-2xl font-black text-[#081C35] mt-0.5">
            ...
            {/* Demographics & Meta */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-[#E8F3FC]/50 p-4 rounded-2xl text-xs mb-5 border border-slate-100">
              <div>
            ...
            {selectedAssessment.summary && (
              <div className="mb-5">
                ...
                <p className="text-sm text-slate-700 bg-[#E8F3FC] p-4 rounded-2xl border border-slate-200/60 leading-relaxed">
                  {selectedAssessment.summary}
                </p>
              </div>
            )}
              ...
              <div className="mb-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedAssessment.detected_indicators.map((ind) => (
                    <span
                      key={ind}
                      className="px-3 py-1 rounded-xl bg-[#FCE1E0] text-[#C4453D] text-xs font-bold border border-[#C4453D]/20"
                    >
                      {ind}
                    </span>
                  ))}
                ...
                <div className="grid sm:grid-cols-2 gap-2">
                  {selectedAssessment.recommendations.map((r) => {
                    ...
                    return (
                      <div
                        key={r}
                        className="flex items-center gap-2.5 bg-[#E8F3FC]/50 border border-slate-100 rounded-xl px-3.5 py-2.5 text-xs font-bold text-[#081C35] capitalize"
                      >
                        <Icon className="w-4 h-4 text-[#0E9F9A]" />
                        <span>{r}</span>
                      </div>
                    );
                  })}
            ...
            <div className="pt-4 border-t border-slate-100 flex items-center justify-between flex-wrap gap-2">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 bg-[#0E9F9A] hover:bg-[#0C8783] text-white text-xs font-bold px-4 py-2.5 rounded-xl transition shadow"
                >
