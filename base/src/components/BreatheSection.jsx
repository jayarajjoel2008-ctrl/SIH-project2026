...
export default function BreatheSection() {
  ...
  return (
    <section id="breathe" className="w-full bg-[#F4F9FD] py-16 sm:py-24 border-y border-slate-100 scroll-mt-16 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-8 items-center">
          ...
          <div className="lg:col-span-6 flex items-center justify-center relative min-h-[380px] sm:min-h-[440px]">
            {/* Outer Ripple Ring 2 */}
            <div
              className={`absolute w-72 h-72 sm:w-96 sm:h-96 rounded-full bg-[#E0F2FE]/50 blur-sm pointer-events-none transition-all ${orb.ring2Scale}`}
            />

            {/* Middle Ripple Ring 1 */}
            <div
              className={`absolute w-60 h-60 sm:w-80 sm:h-80 rounded-full bg-[#BAE6FD]/40 blur-xs pointer-events-none transition-all ${orb.ring1Scale}`}
            />

            {/* Additional soft ambient glow */}
            <div className="absolute w-52 h-52 sm:w-64 sm:h-64 rounded-full bg-[#818CF8]/15 blur-xl pointer-events-none" />
            ...
            <div
              ...
              style={{
                background: "radial-gradient(circle at 35% 30%, #A5B4FC 0%, #6366F1 45%, #4338CA 100%)",
                boxShadow: "0 20px 50px rgba(99, 102, 241, 0.35), inset 0 2px 6px rgba(255, 255, 255, 0.4)"
              }}
            ...
            >
              ...
              <span className="text-xs sm:text-sm font-medium opacity-90 mt-1 tracking-wide text-indigo-100">
                {orb.subtitle}
              </span>
            </div>
          </div>

          {/* Right Column: Typography & Controls matching screenshot */}
          <div className="lg:col-span-6 space-y-6 text-left">
            {/* Red dot badge: • TWO-MINUTE RESET */}
            <div className="inline-flex items-center gap-2 text-xs font-bold tracking-widest text-[#5C6479] uppercase">
              <span className="w-2 h-2 rounded-full bg-[#E06D53] shrink-0 animate-pulse" />
              TWO-MINUTE RESET
            </div>

            {/* Heading matching screenshot */}
            <h2 className="text-4xl sm:text-5xl lg:text-6xl font-black text-[#1E293B] tracking-tight leading-[1.12]">
              A little more room
              <span className="block font-serif italic font-normal text-[#D96B43] mt-1">
                inside the moment.
              </span>
            </h2>
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs sm:text-sm text-slate-500 font-medium pt-1">
              <span className="inline-flex items-center gap-1.5 text-slate-700">
                <SlidersHorizontal className="w-4 h-4 text-[#584CE4]" />
                4 seconds in · 6 seconds out
              </span>
              <span className="text-slate-300">•</span>
              <span className="inline-flex items-center gap-1.5 text-slate-700">
                <Sparkles className="w-4 h-4 text-amber-500" />
                <strong className="text-slate-900">{completedCycles}</strong> cycles completed
              </span>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              <button
                onClick={handleStartToggle}
                className="inline-flex items-center gap-2.5 bg-[#584CE4] hover:bg-[#473BC7] text-white font-bold px-7 py-3.5 rounded-xl shadow-lg shadow-indigo-200 text-base transition-all hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
              >
                {isRunning ? (
                  <>
